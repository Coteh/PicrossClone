            ...
            if (font != null) {
                ...
                for (int i = 0; i < boardHeight; i++) {
                    ...
                    for (int j = colCountLength - 1; j >= 0; j--) {
                        _spriteBatch.DrawString(font, "" + countedStrArr[i + boardWidth].countedData[j], new Vector2(topPosStart.X + (i * spacing), 16 + topPosStart.Y - (spacing * (colCountLength - j))), Alignment.Top, Color.Black);
                    }
                }
            }
        }
