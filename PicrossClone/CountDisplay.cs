...
namespace PicrossClone {
    public class CountDisplay {
        ...
        public void SetPositions(Vector2 _leftSidePos, Vector2 _topSidePos){
            leftStart = _leftSidePos;
            topStart = _topSidePos;
        }

        public void Draw(SpriteBatch _spriteBatch) {
            for (int i = 0; i < boardWidth; i++) {
                _spriteBatch.DrawString(Assets.font, countedStrArr[i], new Vector2(leftStart.X, leftStart.Y + (i * spacing)), Alignment.Left, Color.Black);
            }
            for (int i = 0; i < boardHeight; i++) {
                int colCountLength = countedStrArr[i + boardWidth].Length;
                for (int j = colCountLength - 1; j >= 0; j--) {
                    _spriteBatch.DrawString(Assets.font, countedStrArr[i + boardWidth][j].ToString(), new Vector2(topStart.X + (i * spacing), 16 + topStart.Y - (spacing * (colCountLength - j))), Alignment.Top, Color.Black);
                }
            }
        }
    }
